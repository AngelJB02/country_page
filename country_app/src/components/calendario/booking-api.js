...
export async function fetchUserBookings(clienteId) {
  const res = await fetch(`http://192.168.1.68:3001/api/reservas/my-reservations/${clienteId}`);
  if (!res.ok) throw new Error('Error al obtener reservas');
  return res.json();
}

export async function createBooking({ cliente_id, clase_id, fecha, hora_inicio }) {
  const res = await fetch('http://192.168.1.68:3001/api/reservas/book', {
    method: 'POST',
...
export async function fetchWeekBookings(fechaInicio, fechaFin, clienteId) {
  ...
  const res = await fetch(`http://192.168.1.68:3001/api/reservas/week?fecha_inicio=${fechaInicio}&fecha_fin=${fechaFin}`);
...
export async function cancelBooking(reservaId, clienteId) {
  const res = await fetch(`http://192.168.1.68:3001/api/reservas/${reservaId}/cancel/${clienteId}`, {
    method: 'PUT',
  });
  const data = await res.json();
  if (!res.ok) {
