...
const InstructorasAdmin = () => {
  ...
  const openDeleteModal = async (instructor) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/instructoras/${instructor.id}/reservas-activas`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (listaReasignaciones.length > 0) {
        const reasignarRes = await fetch(`http://localhost:3001/api/instructoras/${instructorToDelete.id}/reasignar`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const deleteRes = await fetch(`http://localhost:3001/api/instructoras/${instructorToDelete.id}`, {
        method: "DELETE"
      });
      if (!deleteRes.ok) {
