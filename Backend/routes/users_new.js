...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (correo && correo.trim() !== '') {
      try {
        await axios.post('https://elrefugiocountryclub.com/api/pi/email/send-credentials', {
          email: correo.trim(),
          nombre: nombre,
          username: username,
          password: password,
...
router.patch('/update-password/:id', async (req, res) => {
  ...
  try {
    ...
    if (user.correo && user.correo.trim() !== '') {
      try {
        await axios.post('https://elrefugiocountryclub.com/api/pi/email/send-updated-credentials', {
